...
$(function () {
	...
	$("#registForm").submit(function () {
		...
		var date_pattern = /^(19|20)\d{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[0-1])$/; 
		if(!date_pattern.test(event_end_time)) {
			$("#event_end_time").parent().find("span").remove();
			html = "<span style='color:red;'>이벤트 종료일 형식을 확인해주세요.</span>";
			$("#event_end_time").parent().append(html);
			return false;
		}
		$("#event_end_time").parent().find("span").remove();
		...
		console.log(event_main_image);
		if (event_main_image == "선택된 파일 없음") {
		...
		// 상세이미지 없을 경우 처리
		var event_content_image = $(".attach-del1").val();
		if (event_content_image == null || event_content_image == "") {
				...
				<%@ include file="/WEB-INF/views/include/admin_side_menu.jsp"%>
					<div class="col-lg-9 col-md-8 col-12">
						...
						<!-- -------- 페이지별 바뀌는 부분  코딩 필요-->
								...
								<!--  페이지별 내용 -->
								<form role="form" action="" method="post" id="registForm">
									...
									<div class="form-group">
										...
										<textarea rows="5" id="event_content" name="event_content" required></textarea>
									</div>
									<div class="form-group">
										<label for="event_start_date"><strong>이벤트 시작일</strong></label>
										<input type="text" class="form-control" id="event_start_date" name="event_start_date" placeholder="ex)2020-07-07" required/>
									</div>
									<div class="form-group">
										<label for="event_end_time"><strong>이벤트 종료일</strong></label>
										<input type="text" class="form-control" id="event_end_time" name="event_end_times" placeholder="ex)2020-07-07" required/>
									</div>
									...
									<div class="form-group">
										...
										<div id="event_thumb_image_div" style="width:300px;height:auto;"></div>
									</div>
									<div class="form-group"> 
